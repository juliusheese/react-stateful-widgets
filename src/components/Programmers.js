...
export default function Programmers() {
  ...
  let [programmer, setProg] = useState(listOfAwesome);
  let [fProgrammer, setF] = useState(null);
  const getNameOfFeatured = () => {
    ...
    return programmer[fProgrammer - 1].name;
  };

  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: fProgrammer !== null ? 'gold' : 'royalblue', // 🤔
  };
      ...
      <div className='programmers'>
        {
          ...
          programmer.map(dev =>
            <div key={dev.id}>
              {dev.name} <button onClick={() => { setF(dev.id)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
